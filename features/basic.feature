Feature: Basic words that can be made from these letters search
  ...
  Scenario: Words without these letters in - Words in the dictionary are returned if they're able to be made from the letters in the tiles also check the order is by score then alphabetical
    Given I have loaded the dictionary
    When I visit the homepage
    And I fill in the letters "-G" into the form
    And search for words
    Then I should see the following words:
      | Word | Score |
      | ACT  | 6     |
  ...
  Scenario: Words without these letters in - case insensitive
    Given I have loaded the dictionary
    When I visit the homepage
    And I fill in the letters "-g" into the form
    And search for words
